...
| 前端入口                 | 已有用户端：`/apps`、`/apps/my`、`/apps/team`、`/apps/:appId`、`/apps/:appId/app`、`/apps/:appId/app/:pageKey`；后台模块中心统一使用 `/settings/admin/modules` 及其分区子路由，不保留旧 URL。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| 数据库依赖               | 已有：`module_apps`、`module_app_versions`、`module_app_pages`、`module_app_actions`、`module_app_entitlements`、`module_app_installations`、`module_app_records`、`module_app_record_events`、`module_app_runs`、`module_app_artifacts`、`module_app_artifact_cleanup_jobs`、`module_app_audit_logs`、`module_app_packages`、`module_app_package_uploads`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| 主要相关文件             | `packages/types/src/moduleApp.ts`、`packages/database/src/schemas/moduleApp.ts`、`packages/database/src/models/moduleApp.ts`、`packages/database/src/models/moduleAppPackageUpload.ts`、`packages/database/src/models/moduleAppArtifactCleanup.ts`、`packages/business-server/src/module-apps/permission.ts`、`packages/business-server/src/module-apps/audit.ts`、`apps/server/src/services/moduleAppPackage`、`apps/server/src/services/moduleAppArtifactCleanup`、`apps/server/src/routers/lambda/moduleApp.ts`、`packages/business-server/src/lambda-routers/admin/moduleApps.ts`、`src/app/(backend)/api/admin/maintenance/route.ts`、`src/services/moduleApp.ts`、`src/services/adminCommercial.ts`、`src/features/ModuleAppMarket`、`src/features/ModuleAppRuntime`、`src/features/Admin/moduleApps`、`src/routes/(main)/apps`、`src/routes/(main)/admin/modules`                                          |
| 维护风险                 | 高                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 是否建议重构             | 是                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 是否需要补测试           | 是                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
- Frontend entries: the admin review queue is available at `/settings/admin/modules/reviews`. P1 originally had no upload UI; P3 now provides the current `/apps/my` upload flow. Admin package review wrappers remain under `adminCommercialService.moduleApps.*Package`.
...
- Note: Approval still converts a reviewed manifest into existing Module App records and entitlements only. P3 now provides trusted ZIP parsing and upload UI; immutable package upgrade/rollback, uninstall UX, and installed-app pagination are implemented, while fixture-backed staging browser acceptance remains pending.
...
- Status: experimental
- Frontend entry: `/settings/admin/modules/finance/*`; billing limits remain under each app's entitlement editor.
...
- Backend-managed gateway readiness: NewAPI instances, provider credentials, enabled chat models, payment providers, Module App payment enablement, and callback origin are managed in the application administration center. Legacy server environment fallbacks remain migration-only compatibility and are surfaced to administrators until equivalent backend settings are saved. The Module App runtime page exposes secret-free readiness counts and method identifiers; GitHub workflows do not duplicate business provider credentials.
...
- Maintenance boundary: the user router is composed from market, runtime, data, workflow, and commerce procedure records, while the database model inherits catalog, installation, execution, and audit ownership layers. Contract tests preserve 32 router procedures and the model ownership surface across this decomposition.
- Remaining release blockers: administrator-reviewed live AI and payment acceptance evidence, authenticated browser install/runtime/workflow/team/license/payment scenarios, and staging blue-green switch/rollback smoke evidence. Runtime invocation, privileged workflow, schedule dispatch, payment mutation, payout recording, and public execution flags must remain disabled until those gates pass.

#### Module App Production Completion (2026-07-14)

- Status: implementation-complete behind disabled production controls; credentialed production acceptance remains blocked.
- Commerce: paid settlement now issues or reactivates the matching installation in the same transaction as the license. Active purchased licenses participate in ordinary entitlement checks. The market supports free personal/workspace installation, paid Alipay computer-website payment form submission, pending-payment continuation, scoped uninstall, and retry-safe order creation. Admin Module Apps includes contract-aligned product creation, product updates, snapshot controls, and active-price rotation without rewriting price history.
- Action runtime: `runModuleAppAction` receives billing, artifact storage, declared/decrypted installation secrets, immutable workflow definitions, the workflow engine, and an allowlisted server-action adapter explicitly. Privileged runtime types reject execution while `MODULE_APP_EXECUTION_ENABLED=false`; `notifications.create` is the only enabled server action and uses a durable dedupe key.
- Host-rendered pages: Module App `list` and `form` pages now use the existing record procedures through schema-driven `RecordList`, `RecordForm`, and `ScopeSwitch` components. The list has loading, empty, retry, edit-adapter, and bounded server-pagination states using `limit`, `offset`, and `total`. Create/edit forms support text, textarea, number, boolean, select, and date fields and revalidate every matching SWR record-list page after mutation. Unsupported/custom pages continue through the isolated sandbox.
...
- Outbound HTTP boundary: every declared `runtime.outboundHosts` entry requires an administrator classification of `general`, `ai`, or `payment`. Only `general` hosts may be used by SDK `http.fetch`, legacy `api_action`, or workflow HTTP nodes; AI and payment traffic must use the platform gateways. Missing, malformed, or incomplete review policy fails closed, so versions approved before classification cannot make direct external requests until a new version is reviewed. Redirects are denied, forbidden transport headers and oversized request bodies are rejected, and response reads stop at 1 MiB.
...
- Deployment relay: before release promotion, CI prunes unused remote images, relays the immutable Worker digest and the digest-pinned `postgres:17-alpine` migration client through OCI archives, imports them into the production containerd store, and verifies both local references.
...
- Build ownership: migration `0144_add_module_app_build_leases.sql` adds claim tokens, lease expiry, attempt counts, and next-attempt scheduling to `module_app_builds`. `@lobechat/module-app-build` owns reviewed-source validation, deterministic artifact creation, S3 staging/promotion verification, and atomic artifact-root materialization. `apps/module-worker` owns PostgreSQL polling, lease renewal, retry/failure transitions, cleanup, health state, and graceful shutdown.
- Runtime dependencies: the worker requires a container-reachable PostgreSQL `DATABASE_URL`, normally `comhub-paradedb:5432`, and joins `COMHUB_PLATFORM_NETWORK` (default `paradedb_default`). Host preflight uses the separate loopback-only `COMHUB_MODULE_WORKER_PREFLIGHT_DATABASE_URL`, normally `127.0.0.1:15432`. Migration checks run through a hardened temporary `postgres:17-alpine` client container with host networking, so the host does not require `psql` and the worker itself does not use host networking. The worker also requires existing S3-compatible object storage through `S3_ENDPOINT`, `S3_BUCKET`, `S3_ACCESS_KEY_ID`, and `S3_SECRET_ACCESS_KEY`, plus an operator-owned `MODULE_APP_ARTIFACT_ROOT`. The deployment host requires Docker Compose and `flock`. If the worker root `.env` is absent, CI bootstraps it from the running `comhub-app` container; an existing `.env` is never overwritten. GitHub production secrets must include the existing SSH private key and `COMHUB_SSH_KNOWN_HOSTS` containing the pinned host-key entries; Worker deployment fails closed when the pinned content is empty and never learns host identity with `ssh-keyscan`.
- Production layout: deployment state lives at `/www/compose/comhub/module-worker`. Root `.env`, `.previous-image`, and `.deploy.lock` remain operator/runtime-owned. Every immutable `releases/<release-id>` directory contains `compose.yml`, `deploy.sh`, and `rollback.sh` plus exact `.env -> ../../.env` and `.previous-image -> ../../.previous-image` symlinks back to root state. CI stages the three assets in the exact unique `releases/.incoming-<release-id>` path, removes only that path on local SCP or remote validation failure, verifies exact membership, shell syntax, and rendered Compose configuration under the root `flock`, then moves the complete release and atomically flips only `current`. On the first deployment, exactly three legacy root assets are copied into a complete executable bootstrap release, zero assets are treated as a clean install with no previous target, and any partial legacy set fails closed. Before execution, CI atomically moves any root `.previous-image` to the exact `.previous-image.before-<release-id>` backup. `current/deploy.sh` resolves the physical directory containing each release-state link before applying a relative target, so `current/../..` cannot collapse into the parent ComHub directory; it atomically records the current immutable image at the module-worker root without replacing the release link. If no current image is recordable, CI restores the pre-deploy state even after a successful deploy. A recorded deploy failure with a previous release restores that `current`, invokes its rollback script, and then restores the pre-deploy root state. On a clean host with no previous release, a recorded failure invokes the complete promoted release's rollback script before removing `current`, then restores the prior root state while preserving the original failure status. Unexpected failures restore the exact backup when the root target is absent; otherwise they preserve the backup and report its exact path. Root `.env` is never copied, overwritten, or deleted. Top-level same-ref workflow concurrency and both production jobs use `cancel-in-progress: false` with `queue: max`; the production jobs additionally share static group `comhub-production-deploy` across refs, so pending runs and deployments remain queued rather than being cancelled or replaced. The worker bind-mounts `MODULE_APP_ARTIFACT_ROOT` read-write at `/runtime/artifacts`; the independently deployed `module-runtime` must mount the identical host path read-only. The worker joins the existing external platform network and does not publish ports.
- Retry and health behavior: one build is processed at a time with a 60-second lease renewed every 20 seconds. Idle polling runs every 5 seconds. Retryable failures are scheduled after 30 seconds, 2 minutes, and 10 minutes, with the fourth attempt terminal; permanent failures are terminal immediately. Health requires a state file newer than 30 seconds plus a writable artifact-root probe. Compose checks health every 10 seconds with a 5-second timeout, 15-second start period, and three retries; shutdown stops new claims and bounds drain time to 40 seconds.
- Manual deployment: `flock -x /www/compose/comhub/module-worker/.deploy.lock -c '/www/compose/comhub/module-worker/current/deploy.sh ghcr.io/<owner>/<repository>-module-worker:sha-<12-character-sha>'`. Manual rollback: `flock -x /www/compose/comhub/module-worker/.deploy.lock -c '/www/compose/comhub/module-worker/current/rollback.sh'`. The server must provide `flock`; automation and operators use the exact same root lock and current release scripts. Manual deploy atomically updates the root `.previous-image` target while preserving the current release's exact symlink. Both commands validate the immutable image/deployment invariants; neither calls the main ComHub deploy script, changes Nginx/certificates, or switches blue-green traffic.
- Publication boundary: a build becoming `ready` means its deterministic artifact was verified, promoted, and materialized. It does not publish the Module App, enable execution, enable runtime invocation, or turn on any privileged workflow, schedule, payment, payout, or public-execution control. All eight production mutation flags remain `false` until their separate release gates pass.

#### Module App Installation Readiness

- Status: experimental; active for installed-app lists, detail views, desktop launch actions, and the mobile application launcher.
- Runtime rule: manifest v2 installations are openable only when the build status is `ready` and the build artifact key and SHA-256 exactly match the immutable installed-version snapshot. Legacy built-in applications and non-v2 manifests remain openable by default. The launch API independently enforces the same build boundary.
- Configuration rule: required credential keys are derived in batches from actions belonging to the installed version, then compared with installation-scoped secret metadata. Missing credentials are reported without blocking launch because some pages and actions may not consume them; malformed declarations are reported as a configuration issue.
- User experience: installed cards and detail pages expose configuration/runtime state, retain detail access when runtime is unavailable, disable only the open action for unavailable runtimes, and refresh readiness after a credential is added, rotated, or deleted. Workspace members who cannot manage secrets receive an administrator handoff message. The mobile launcher keeps unavailable installations visible but disabled.
- Query boundary: installed lists load action declarations and configured-secret metadata in batches rather than per-card queries. Readiness always follows the active installation version, never the latest published version.
- Commerce boundary: this readiness work does not change payments, pricing, billing, subscriptions, refunds, licenses, or Alipay behavior.
...
- Status: experimental
- Description: P2-A adds a usable modular admin center at `/settings/admin/modules` for listing apps, creating/editing metadata, pages, actions, entitlements, billing config, publish state, and read-only installs/records/runs/artifacts/audit inspection.
- Frontend entries: `/settings/admin/modules`, `/settings/admin/modules/apps`, `/settings/admin/modules/reviews`, `/settings/admin/modules/operations/*`, and `/settings/admin/modules/finance/*`.
...
- Maintenance risk: high. Durable quotas, abandoned-upload cleanup, static malware/payload scanning, legacy rescans, approval gates, immutable upgrades, and uninstall storage lifecycle are now active; external antivirus, package signing/provenance, production runtime enablement, and fixture-backed staging browser acceptance remain outstanding.
...
- Maintenance risk: medium for the overview; the broader package lifecycle remains high risk. Quotas, static scanning, abandoned-upload cleanup, immutable upgrades, uninstall storage lifecycle, and installed-list pagination are active, while external antivirus, package signing/provenance, production runtime enablement, and fixture-backed staging browser acceptance remain outstanding.
...
- Frontend entries: `/apps/my` displays quota, expiry, size, and security rejection feedback; `/settings/admin/modules/reviews` displays scan state and exposes explicit Scan/Approve/Reject actions; the system maintenance result displays cleanup success/failure counts.
...
- Deferred P1/P2: external antivirus, signed packages/provenance, production runtime enablement, and fixture-backed staging browser acceptance.
...
- Frontend entry: `/settings/admin/modules/reviews` displays package build status and bounded failure codes. Existing `/apps/my` and marketplace routes remain unchanged in this stage.
...
- Database dependencies: `module_app_packages`, `module_app_versions`, `module_app_builds`, `module_app_installation_secrets`, `module_app_installation_version_refs`, and additive migrations `0136_add_module_app_build_runtime.sql` and `0168_module_app_installation_lifecycle.sql`.
...
- Refactor recommendation: keep immutable installation version transitions, artifact retention, runtime authorization, and commerce settlement in separate tested boundaries.
- Test gap: focused contracts, real database transitions, installation upgrade/rollback/uninstall lifecycle, storage orchestration, admin API, UI, capability SDK, runtime service contract, and type-check coverage exist. Build worker integration, browser iframe isolation, production sandbox probes, artifact-mount integration, and end-to-end rollout tests remain pending.
...
- Refactor recommendation: keep adapters small and injectable. The business gateway retains injectable asynchronous guard contracts; the server adapter owns Redis and local-fallback policy.
- Test coverage: method permission denial, browser secret redaction, runtime-only decryption, installation file isolation, SSRF denial, bounded HTTP responses, same-process and shared-backend replay denial, same-process and shared-backend notification rate limits, Redis atomic-command adapters, database installation/secret isolation, browser-only user route, type-check, and lint.
...
- Known limitation: Redis-backed guards are cross-instance only when Redis is configured and reachable. Intentional Redis disablement falls back to process-local protection for development and single-instance operation. Production enablement remains blocked until Redis configuration plus sandbox/E2E probes are verified.
- Docker deployment: no Docker or bind-mount change. Runtime deployment remains disabled.
...
- Configuration and environment: `MODULE_APP_EXECUTION_ENABLED` remains the master switch; `MODULE_APP_RUNTIME_INVOCATION_ENABLED`, `MODULE_APP_WORKFLOW_PRIVILEGED_EXECUTORS_ENABLED`, `MODULE_APP_SCHEDULE_DISPATCH_ENABLED`, `MODULE_APP_PUBLIC_EXECUTION_ENABLED`, `MODULE_APP_ALIPAY_PAYMENT_CREATION_ENABLED`, `MODULE_APP_ALIPAY_AUTO_SETTLEMENT_ENABLED`, and `MODULE_APP_PUBLISHER_PAYOUT_RECORDING_ENABLED` independently guard new mutations. `MODULE_APP_RUNTIME_APP_ALLOWLIST` and `MODULE_APP_PUBLISHER_ALLOWLIST` are bounded comma-separated rollout lists. Every new switch defaults false and empty lists deny rollout. Runtime credentials remain mandatory when enabled and are never hardcoded. `MODULE_APP_RUNTIME_JWKS` is the public verification-only projection of `JWKS_KEY`; it must contain an importable RSA/RS256 public key and must not expose private RSA parameters. Container execution additionally requires `MODULE_APP_RUNTIME_ROOTLESS_DOCKER_SOCKET`, its numeric `MODULE_APP_RUNTIME_ROOTLESS_DOCKER_GID`, and `MODULE_APP_RUNTIME_DOCKER_ARTIFACT_ROOT`, which must be a normalized daemon-visible absolute POSIX path other than `/`, mounted read-only at `/runtime/artifacts`, and shared with the Module App Worker.
...
- Test coverage: fixed invocation validation, custom-image and timeout denial, bounded logs, runtime-versus-browser capability enforcement, signed artifact-hash matching and mismatch denial, disabled-client behavior, internal endpoint routing, immutable static asset serving, incomplete credential rejection, independent fail-closed controls, rollout allowlists, and bounded OpenTelemetry dimensions. Unit tests inject a launcher and do not execute uploaded or host commands; real-container and staging gates remain pending.
...
- Docker deployment: fixed non-root Dockerfiles run as UID/GID `10001`. The runtime accepts only the dedicated rootless Docker socket at `/run/module-app-docker/docker.sock`; direct access to `/var/run/docker.sock` is rejected. Production deployment verifies the rootless daemon, read-only socket bind, immutable daemon-visible artifact path, and the identical read-only artifact bind before accepting the runtime service. Runtime invocation remains disabled until the remaining network, seccomp, rollback, and browser acceptance gates pass.
...
- Description: Adds `moduleApp.getLaunchContext`, active installation/build lookup, current plan and workspace revalidation, a five-minute browser capability, a main-site iframe shell with loading/denied/build/runtime/error/retry states, and host-rendered record list/form pages selected from the reviewed runtime manifest.
- Frontend entry: `/apps/:appId/app` and `/apps/:appId/app/:pageKey`; workspace launches use the optional `workspaceId` query parameter.
- Core components: `src/features/ModuleAppRuntime/index.tsx`, `PageRenderer.tsx`, `SandboxFrame.tsx`, `RecordList.tsx`, `RecordForm.tsx`, `ScopeSwitch.tsx`, and SDK `waitForModuleAppLaunch`.
...
- Configuration and environment: public launch additionally requires `MODULE_APP_PUBLIC_EXECUTION_ENABLED` and an application or Publisher entry in the rollout allowlists; executable actions require `MODULE_APP_RUNTIME_INVOCATION_ENABLED`. The runtime service separately uses `MODULE_APP_RUNTIME_INTERNAL_URL`, `MODULE_APP_RUNTIME_INTERNAL_TOKEN`, and `MODULE_APP_RUNTIME_JWKS`.
...
- Status: experimental
- Description: Adds bounded app commerce contracts, personal/workspace credit reservations, one entitlement authority, real content-generation execution through the current user's AI router, audited developer revenue settlement, and honest user purchase/cost UX. Platform-managed AI usage is quoted through the existing official/model pricing path and settled once after applying separate app and action multiplier snapshots. User-managed provider credentials remain outside platform AI charging.
- Frontend entries: `/apps`, `/apps/my`, `/apps/team`, `/apps/:appId`, `/apps/:appId/app`, `/apps/:appId/app/:pageKey`, and `/settings/admin/modules/finance/*`. The app detail page isolates personal/team catalog, order, license, and launch state by optional `workspaceId`; the runtime page displays the latest persisted cost snapshot.
- Core components: `src/features/ModuleAppMarket/AppCard.tsx`, `AppDetail.tsx`, `PurchaseModal.tsx`, `src/features/ModuleAppRuntime/RecentRunResult.tsx`, `RunResultPanel.tsx`, and `src/features/Admin/moduleApps/CommerceTable.tsx`.
- Backend API and services: `lambda.moduleApp.listCatalog`, `quoteProduct`, `createOrder`, `cancelOrder`, `listOrders`, `getLicense`, `listRuns`, and `runAction`; `createModuleAppTextGenerator`; `resolveModuleAppEntitlement`; ModelRuntime `onRouteResolved`; quote-only `quoteCommercialAiUsage`; admin finance settlement/refund/revenue procedures; and the append-only Module App revenue service.
- Database dependencies: migrations `0138_add_module_app_commerce.sql` and `0139_add_module_app_revenue.sql`; products/prices/orders/licenses/subscriptions; revenue entries and settlement batches; `credit_reservations`; workspace credit accounts/ledgers; existing personal credit accounts/ledgers; Module App entitlement/install/run tables; and model/provider pricing tables. Published-app catalog listing, server quotes, authenticated personal/workspace pending-order creation, purchaser cancellation, row-locked idempotent settlement, personal/workspace license resolution, trial/monthly/yearly periods, paid-order refund revocation, bounded order history, license lookup, revenue accrual/reversal, and delayed settlement batches are implemented. Workspace operations require current membership. Settlement, refunds, and revenue settlement require admin finance permission and emit audit events; they remain outside user-facing routes.
...
- Main files: `apps/server/src/services/moduleAppAi/index.ts`, `apps/server/src/modules/ModelRuntime/index.ts`, `apps/server/src/routers/lambda/moduleApp.ts`, `packages/business-server/src/commercialBilling.ts`, `packages/business-server/src/model-runtime.ts`, `packages/business-server/src/module-apps/entitlement.ts`, `packages/business-server/src/module-apps/runModuleAppAction.ts`, `packages/business-server/src/module-apps/revenue.ts`, `packages/database/src/models/moduleAppCredit.ts`, `packages/database/src/models/moduleAppCommerce.ts`, `packages/database/src/models/commercial.ts`, `packages/database/src/models/moduleApp.ts`, `src/features/ModuleAppMarket/`, `src/features/ModuleAppRuntime/`, and `src/features/Admin/moduleApps/CommerceTable.tsx`.
...
- Test coverage: entitlement matrix, router enforcement, policy-preserving billing bypass, NewAPI primary/failover route snapshots, multiplier precision, BYOK no-charge behavior, pre-provider release, reservation settlement/idempotency/concurrency, order/license/refund/revenue transitions, purchase status/scope/promotion UX, persisted runtime cost snapshots, and focused type/lint verification.
- Commerce snapshot boundary: every order freezes server-derived price, currency, billing period, trial, promotion, scope, seat count, app multiplier, revenue-share rate, and terms version. Applications and browsers cannot submit ledger amounts or settlement results. The idempotent settlement model is shared by manual admin and verified Alipay notification adapters.
- Payment adapter status: Alipay computer website payment is implemented for server-owned payment-form creation and verified notification settlement. Production payment creation and automatic settlement remain disabled until credentialed sandbox/staging acceptance passes; only an active server-resolved license enables app launch.
- Known limitation: the UI can render a server-provided pre-run estimate, but no general action-estimate endpoint exists yet. Actual costs are visible from immutable persisted run snapshots; the browser never estimates or recomputes prices itself.
- Docker deployment: no compose or volume change. Apply the current migration set, including `0145` for order idempotency, before enabling this path; production Module App execution remains disabled until the separate runtime security acceptance gate passes.
