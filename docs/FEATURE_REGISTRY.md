...
- Production layout: deployment state lives at `/www/compose/comhub/module-worker`. Root `.env`, `.previous-image`, and `.deploy.lock` remain operator/runtime-owned. Every immutable `releases/<release-id>` directory contains `compose.yml`, `deploy.sh`, and `rollback.sh` plus `.env` and `.previous-image` symlinks back to root state. CI stages the three assets in the exact unique `releases/.incoming-<release-id>` path, removes only that path on local SCP or remote validation failure, verifies exact membership, shell syntax, and rendered Compose configuration under the root `flock`, then moves the complete release and atomically flips only `current`. Existing legacy root assets are copied into a complete executable bootstrap release before the first `current` conversion. A failed post-promotion deploy atomically restores the previous `current`; when Task 9 recorded the current image, the workflow attempts `current/rollback.sh` under the restored release and then restores the pre-deploy root `.previous-image` state. Root `.env` is never copied, overwritten, or deleted. Main and Worker production jobs share static group `comhub-production-deploy` with `cancel-in-progress: false` and `queue: max`, so pending production deployments remain queued rather than being cancelled or replaced. The worker bind-mounts `MODULE_APP_ARTIFACT_ROOT` read-write at `/runtime/artifacts`; the independently deployed `module-runtime` must mount the identical host path read-only. The worker joins the existing external platform network and does not publish ports.
