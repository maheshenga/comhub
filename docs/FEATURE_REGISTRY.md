...
- Database dependencies: migration `0138_add_module_app_commerce.sql`, products/prices/orders/licenses/subscriptions, `credit_reservations`, workspace credit accounts/ledgers, existing personal credit accounts/ledgers, Module App entitlement/install/run tables, and model/provider pricing tables. Published-app catalog listing, server quotes, authenticated personal/workspace pending-order creation, purchaser cancellation, row-locked idempotent settlement, personal/workspace license resolution, trial/monthly/yearly periods, paid-order refund revocation, bounded order history, and license lookup are implemented. Workspace operations require current membership. Settlement and refunds require admin finance permission and emit audit events; they remain outside user-facing routes.
