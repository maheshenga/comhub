...
- Production layout: deployment state lives at `/www/compose/comhub/module-worker`. CI stages only `compose.yml`, `deploy.sh`, and `rollback.sh` in a unique `releases/.incoming-<release-id>` directory. Under `/www/compose/comhub/module-worker/.deploy.lock`, the remote step verifies exact file membership, shell syntax, and rendered Compose configuration, moves the directory to `releases/<release-id>`, atomically flips `current`, and atomically replaces root `compose.yml`, `deploy.sh`, and `rollback.sh` with stable `current/*` symlinks. Root `.env` and `.previous-image` remain operator/runtime-owned and are never copied or overwritten. Main and Worker production deploy jobs share one non-cancelling concurrency group. The worker bind-mounts `MODULE_APP_ARTIFACT_ROOT` read-write at `/runtime/artifacts`; the independently deployed `module-runtime` must mount the identical host path read-only. The worker joins the existing external platform network and does not publish ports.
...
- Manual deployment: `cd /www/compose/comhub/module-worker && ./deploy.sh ghcr.io/<owner>/<repository>-module-worker:sha-<12-character-sha>`. Manual rollback: `cd /www/compose/comhub/module-worker && ./rollback.sh`. Both commands validate the immutable image and deployment invariants; neither calls the main ComHub deploy script, changes Nginx/certificates, or switches blue-green traffic.
