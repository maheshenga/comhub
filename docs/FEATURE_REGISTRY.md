...
- Production layout: deployment state lives at `/www/compose/comhub/module-worker`. Root `.env`, `.previous-image`, and `.deploy.lock` remain operator/runtime-owned. Every immutable `releases/<release-id>` directory contains `compose.yml`, `deploy.sh`, and `rollback.sh` plus exact `.env -> ../../.env` and `.previous-image -> ../../.previous-image` symlinks back to root state. CI stages the three assets in the exact unique `releases/.incoming-<release-id>` path, removes only that path on local SCP or remote validation failure, verifies exact membership, shell syntax, and rendered Compose configuration under the root `flock`, then moves the complete release and atomically flips only `current`. Existing legacy root assets are copied into a complete executable bootstrap release before the first `current` conversion. Before execution, CI atomically moves any root `.previous-image` to the exact `.previous-image.before-<release-id>` backup. `current/deploy.sh` resolves the release symlink and atomically records the current immutable image at the root target without replacing the release link. If no current image is recordable, CI restores the pre-deploy state even after a successful deploy. A recorded deploy failure restores the previous `current`, conditionally invokes restored `current/rollback.sh` against the recorded root state, and then restores the pre-deploy root state. Unexpected failures restore the exact backup when the root target is absent; otherwise they preserve the backup and report its exact path. Root `.env` is never copied, overwritten, or deleted. Main and Worker production jobs share static group `comhub-production-deploy` with `cancel-in-progress: false` and `queue: max`, so pending production deployments remain queued rather than being cancelled or replaced. The worker bind-mounts `MODULE_APP_ARTIFACT_ROOT` read-write at `/runtime/artifacts`; the independently deployed `module-runtime` must mount the identical host path read-only. The worker joins the existing external platform network and does not publish ports.
