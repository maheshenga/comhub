...
- Runtime dependencies: the worker requires PostgreSQL through `DATABASE_URL`, existing S3-compatible object storage through `S3_ENDPOINT`, `S3_BUCKET`, `S3_ACCESS_KEY_ID`, and `S3_SECRET_ACCESS_KEY`, plus an operator-owned `MODULE_APP_ARTIFACT_ROOT`. The deployment host also requires Docker Compose, `psql`, and `flock` for preflight and serialized promotion. GitHub production secrets must include the existing SSH private key and `COMHUB_SSH_KNOWN_HOSTS` containing the pinned host-key entries; Worker deployment fails closed when the pinned content is empty and never learns host identity with `ssh-keyscan`.
