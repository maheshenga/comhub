...
- Runtime dependencies: the worker requires PostgreSQL through `DATABASE_URL`, existing S3-compatible object storage through `S3_ENDPOINT`, `S3_BUCKET`, `S3_ACCESS_KEY_ID`, and `S3_SECRET_ACCESS_KEY`, plus an operator-owned `MODULE_APP_ARTIFACT_ROOT`. The deployment host also requires Docker Compose and `psql` for preflight verification.
- Production layout: deployment assets live at `/www/compose/comhub/module-worker`. CI copies only `compose.yml`, `deploy.sh`, and `rollback.sh`; the server-owned `.env` is never overwritten. The worker bind-mounts `MODULE_APP_ARTIFACT_ROOT` read-write at `/runtime/artifacts`; the independently deployed `module-runtime` must mount the identical host path read-only. The worker joins the existing external platform network and does not publish ports.
