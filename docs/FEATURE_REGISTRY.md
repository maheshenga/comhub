...
- Commerce: paid settlement now issues or reactivates the matching installation in the same transaction as the license. Active purchased licenses participate in ordinary entitlement checks. The market supports free personal installation, paid Alipay computer-website payment form submission, pending-payment continuation, and personal uninstall. Admin Module Apps includes product creation, product updates, and active-price rotation without rewriting price history.
...
- Payment adapter status: online payment is not connected. User orders remain pending until audited manual settlement or a future verified payment webhook. Paid-but-not-yet-licensed orders block duplicate checkout; only an active server-resolved license enables app launch.
...
- Docker deployment: no compose or volume change. Apply migrations `0138` and `0139` before enabling this path; production Module App execution remains disabled until the separate runtime security acceptance gate passes.
