...
- Database dependencies: migration `0138_add_module_app_commerce.sql`, products/prices/orders/licenses, `credit_reservations`, workspace credit accounts/ledgers, existing personal credit accounts/ledgers, Module App entitlement/install/run tables, and model/provider pricing tables. Published-app catalog listing, server quotes, authenticated pending-order creation, purchaser cancellation, row-locked idempotent settlement, personal license resolution, paid-order refund revocation, bounded order history, and personal license lookup are implemented. Settlement and refunds require admin finance permission and emit audit events; they remain outside user-facing routes.
