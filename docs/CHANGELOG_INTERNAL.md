...
- MODULE-APP-WORKER-004: Added the non-root `apps/module-worker` image and independent `/www/compose/comhub/module-worker` deployment. CI publishes `${IMAGE_NAME}-module-worker:sha-${GITHUB_SHA::12}` on verified pushes and dispatches, exposes `worker_image_ref`, validates optional manual tags from step environment against Docker tag syntax, and deploys only when the separate `deploy_module_worker` dispatch choice is `true`. Main and Worker production jobs share one non-cancelling concurrency group. Worker SSH requires the pinned `COMHUB_SSH_KNOWN_HOSTS` secret with strict host-key checking. Assets stage under `releases/.incoming-*`, validate under `flock`, promote to `releases/<release-id>`, atomically flip `current`, and expose stable root symlinks while preserving root `.env` and `.previous-image`; the job does not call the main deploy script, modify Nginx/certificates, or switch blue-green traffic.
...
- Operational dependencies: apply migration `0144` before deployment and provide PostgreSQL, S3-compatible storage, and one shared artifact root. Deploy with `cd /www/compose/comhub/module-worker && ./deploy.sh <immutable-worker-image>`; roll back with `cd /www/compose/comhub/module-worker && ./rollback.sh`.
