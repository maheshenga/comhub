...
- MODULE-APP-WORKER-004: Added the non-root `apps/module-worker` image and independent `/www/compose/comhub/module-worker` deployment. CI publishes `${IMAGE_NAME}-module-worker:sha-${GITHUB_SHA::12}` on verified pushes and dispatches, exposes `worker_image_ref`, and deploys only when the separate `deploy_module_worker` dispatch choice is `true`; it does not overwrite `.env`, call the main deploy script, modify Nginx/certificates, or switch blue-green traffic.
