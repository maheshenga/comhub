...
- MODULE-APP-WORKER-001: Added migration `0144_add_module_app_build_leases.sql` and bounded PostgreSQL claim leases with claim tokens, expiry recovery, attempt counts, next-attempt scheduling, renewal, and stale-claim rejection.
- MODULE-APP-WORKER-002: Added `@lobechat/module-app-build` deterministic reviewed-source processing with fixed build profiles, canonical artifact creation, SHA-256 identity, claim-scoped S3 staging, verified promotion, and immutable final keys.
- MODULE-APP-WORKER-003: Added atomic artifact-root materialization with claim-scoped staging cleanup, identity markers, read-only materialized files/directories, and shared mount verification: worker read-write, runtime read-only.
- MODULE-APP-WORKER-004: Added the non-root `apps/module-worker` image and independent `/www/compose/comhub/module-worker` deployment. CI publishes `${IMAGE_NAME}-module-worker:sha-${GITHUB_SHA::12}` on verified pushes and dispatches, exposes `worker_image_ref`, validates optional manual tags from step environment against Docker tag syntax, and deploys only when the separate `deploy_module_worker` dispatch choice is `true`. Top-level same-ref workflow concurrency and both production jobs use `cancel-in-progress: false` with `queue: max`; Main and Worker production jobs additionally share static group `comhub-production-deploy` across refs, preserving pending runs and deployments instead of cancelling or replacing them. Worker SSH requires pinned `COMHUB_SSH_KNOWN_HOSTS` with strict host-key checking. Exact incoming paths are cleaned on SCP/validation failure without globs. On first deployment, all three legacy root assets bootstrap into an executable previous release, no legacy assets produce a clean install with no previous target, and a partial set fails closed. Each release contains the three assets plus exact root-state symlinks. Promotion atomically flips only `current`. CI moves root `.previous-image` to an exact per-release backup before execution, and `current/deploy.sh` resolves each link's physical containing directory before applying relative targets, preventing `current/../..` from escaping the module-worker root. A no-record success restores the pre-state. A recorded failure restores and rolls back the prior release; on a clean host it rolls back through the complete promoted release before removing `current`. Both paths restore the pre-state and preserve the original failure status; unexpected failures either restore the exact backup or preserve and report its exact path. Root `.env` is never copied or overwritten; the job does not call the main deploy script, modify Nginx/certificates, or switch blue-green traffic.
- MODULE-APP-WORKER-005: Added real-container worker verification covering PostgreSQL leases, S3 artifact identity, deterministic builds, runtime serving, health freshness and artifact-root probes, retry exhaustion, cleanup, graceful shutdown, Compose hardening, eight disabled production flags, and the no-push-deployment workflow boundary.
- Operational dependencies: apply migration `0144` before deployment and provide PostgreSQL, S3-compatible storage, one shared artifact root, and server `flock`. Manual deploy: `flock -x /www/compose/comhub/module-worker/.deploy.lock -c '/www/compose/comhub/module-worker/current/deploy.sh <immutable-worker-image>'`; it preserves the release `.previous-image` symlink and atomically updates the root target. Manual rollback: `flock -x /www/compose/comhub/module-worker/.deploy.lock -c '/www/compose/comhub/module-worker/current/rollback.sh'`.
- Release boundary: worker status `ready` verifies and materializes an artifact only. It does not publish an application or enable execution. Production execution, runtime invocation, privileged workflow, schedule, Alipay mutation, payout recording, and public execution controls remain disabled.
...
- MODULE-APP-VERIFY-005: Split the user Module App router into market, runtime, data, workflow, and commerce records and split the database model into catalog, installation, execution, and audit ownership layers. Compatibility tests now lock all 30 procedure identities, query/mutation types, input-schema fingerprints, authentication/database/context middleware boundaries, and the complete 42-method model surface.
