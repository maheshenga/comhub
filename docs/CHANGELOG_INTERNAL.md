...
- MODULE-APP-WORKER-004: Added the non-root `apps/module-worker` image and independent `/www/compose/comhub/module-worker` deployment. CI publishes `${IMAGE_NAME}-module-worker:sha-${GITHUB_SHA::12}` on verified pushes and dispatches, exposes `worker_image_ref`, validates optional manual tags from step environment against Docker tag syntax, and deploys only when the separate `deploy_module_worker` dispatch choice is `true`. Main and Worker production jobs share static group `comhub-production-deploy` with `cancel-in-progress: false` and `queue: max`, preserving pending deployments instead of cancelling or replacing them. Worker SSH requires pinned `COMHUB_SSH_KNOWN_HOSTS` with strict host-key checking. Exact incoming paths are cleaned on SCP/validation failure without globs. Legacy root assets bootstrap into an executable previous release; each release contains the three assets plus exact root-state symlinks. Promotion atomically flips only `current`. CI moves root `.previous-image` to an exact per-release backup before execution, and `current/deploy.sh` resolves the release link and atomically records the immutable current image at the root target without replacing the link. A no-record success restores the pre-state. A recorded failure restores the prior `current`, conditionally attempts restored `current/rollback.sh`, and restores the pre-state; unexpected failures either restore the exact backup or preserve and report its exact path. Root `.env` is never copied or overwritten; the job does not call the main deploy script, modify Nginx/certificates, or switch blue-green traffic.
