...
- 不允许管理员控制用户个人置顶。
- 不新增推荐模型、文稿、图片、PPT 或模块应用的业务数据副本。
- 不重写聊天、群组、文稿、图片、PPT、社区或模块应用的核心业务逻辑。

## 3. 已确认决策

- 采用“移动壳层 + 功能适配器”，不直接把桌面页面压缩到手机尺寸，也不建立独立业务系统。
- “最近”使用紧凑统一列表；置顶、AI 助手群组、助手和话题处于同一视觉层级。
- 置顶由用户个人控制，并复用现有会话 `pinned` 能力实现账号同步。
...
```mermaid
flowchart LR
  Admin["后台移动端配置"] --> Settings["App Settings: mobile section"]
  Settings --> PublicConfig["getPublicMobileConfig"]
  PublicConfig --> Shell["MobileShell"]
  Shell --> Recent["最近适配器"]
  Shell --> Design["设计适配器"]
  Shell --> Discover["发现适配器"]
  Shell --> Apps["应用适配器"]
  Recent --> Sessions["现有会话、群组、话题数据"]
...
- 后台可以配置顺序、卡片标题覆盖、描述覆盖和推荐模型。
- 推荐模型必须从当前可用模型目录选择，并保存 `provider` 与 `model` 标识。
- 卡片显示助手头像、名称、简短描述和“推荐模型”标签。
...
配置存放在独立的 `mobile` App Settings section 中。公开配置采用版本化对象，不并入 `BrandProvider`。
...
后台读取和保存继续复用现有后台 settings 查询与批量更新能力，服务端从 `mobile` section 构建管理读模型和公开读模型。
...
3. 置顶会话对应的普通最近项被移除。
4. 相同 `routePath` 或相同会话与话题组合只保留更新时间较新的项。
