...
// These URLs can be overridden via NEXT_PUBLIC_* env vars for self-hosted / custom branding deployments.
// In Vite SPA builds, sharedRendererDefine forwards NEXT_PUBLIC_* at build time.
// In Next.js SSR, they are read as real env vars at runtime.
// In Electron main process, they are read from process.env at runtime.
...
// Derive domain from OFFICIAL_URL for uses like cookie scoping
const _parseDomain = (url: string): string => {
  try {
...
export const OFFICIAL_DOMAIN = process.env.NEXT_PUBLIC_OFFICIAL_DOMAIN || _parseDomain(OFFICIAL_URL);
