...
export default class RemoteServerConfigCtr extends ControllerModule {
  ...
  async setupSubscriptionWebviewSession(params: { partition: string }) {
    ...
    // Derive URL filter from the configured remote server so it works for custom domains
    const remoteUrl = await this.getRemoteServerUrl();
    let urlFilter = `https://*.lobehub.com/*`;
    if (remoteUrl) {
      try {
        const hostname = new URL(remoteUrl).hostname;
        // Support both the exact hostname and wildcard subdomains
        urlFilter = `https://*.${hostname}/*`;
      } catch {
        // fallback to lobehub.com if URL parsing fails
      }
    }
