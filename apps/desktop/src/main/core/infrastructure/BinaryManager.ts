...
type BinaryDownloader = (url: string, dest: string) => Promise<void>;

interface BinaryManagerOptions {
...
export class BinaryManager {
  private app: App;
  private download: BinaryDownloader;
  ...
  constructor(app: App, options: BinaryManagerOptions = {}) {
    logger.debug('Initializing BinaryManager');
    this.app = app;
    this.download = options.download ?? downloadWithRedirects;
  }
  ...
  async install(name: string, version?: string): Promise<string> {
    ...
    await this.download(releaseUrl, targetPath);
    ...
    // Atomically point the .installed marker at the new version only after the
    // downloaded binary and its post-install steps are both usable.
