...
const { mockExecFile, mockUserDataDir, setUserDataDir } = vi.hoisted(() => {
  const ref = { current: '' };
  return {
    mockExecFile: vi.fn(),
    mockUserDataDir: ref,
...
describe('BinaryManager', () => {
  ...
  describe('install', () => {
    ...
    it('does not mark a managed binary installed when postInstall fails', async () => {
      mockExecFile.mockImplementation((_file, _args, _options, callback) => {
        callback(new Error('post-install failed'));
      });

      const mgr = new BinaryManager(stubApp, {
        download: async (_url, dest) => writeFile(dest, 'downloaded-binary', 'utf8'),
      });
      mgr.register(
        {
          detect: async () => ({ available: false }),
      ...
      await expect(mgr.install('foo')).rejects.toThrow();
      await expect(mgr.findManagedPath('foo')).resolves.toBeNull();
      await expect(mgr.readInstalledVersion('foo')).resolves.toBeNull();
    });
  });
