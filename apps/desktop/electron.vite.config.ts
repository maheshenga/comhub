...
// Sync: if OFFICIAL_CLOUD_SERVER is set but NEXT_PUBLIC_OFFICIAL_URL is not,
// forward it so the Vite renderer build picks up the same URL via sharedRendererDefine.
if (process.env.OFFICIAL_CLOUD_SERVER && !process.env.NEXT_PUBLIC_OFFICIAL_URL) {
  process.env.NEXT_PUBLIC_OFFICIAL_URL = process.env.OFFICIAL_CLOUD_SERVER;
}
