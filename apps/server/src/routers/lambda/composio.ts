...
const composioPluginProcedure = wsCompatProcedure.use(serverDatabase).use(async (opts) => {
  const { ctx } = opts;
...
const composioProcedure = composioPluginProcedure.use(async (opts) => {
  ...
  return opts.next({
    ctx: { ...ctx, composioClient, connectorModel, connectorToolModel },
  });
});
...
export const composioRouter = router({
  createConnection: composioWriteProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      authConfigId ||= getServerComposioAuthConfigId(identifier);
      if (!authConfigId) {
