...
import { computeMessageCostSplit, computeMessageCostSplitWithPricing } from './cost';
...
describe('computeMessageCostSplit', () => {
  it('uses supplied server pricing for custom model cost split', () => {
    ...
    const split = computeMessageCostSplitWithPricing(usage, pricing, 0);

    expect(split.inputCost).toBe(2);
    expect(split.outputCost).toBe(2);
