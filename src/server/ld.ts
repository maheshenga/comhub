...
export class Ld {
  generate({
    ...
    return {
      ...
      '@graph': [
        this.genWebSite(brandName),
        article?.enable && this.genArticle({ ...article, brandName, date, description, locale, title, url }),
        webpage?.enable &&
          this.genWebPage({
            ...webpage,
            brandName,
  ...
  genWebPage({
    ...
    const baseInfo: any = {
      ...
      'name': this.fixTitle(title, brandName),
      'primaryImageOfPage': {
        '@id': this.getId(fixedUrl, '#primaryimage'),
      },
      'thumbnailUrl': image,
  ...
  genWebSite(brandName?: string) {
    const baseInfo: any = {
      '@id': this.getId(OFFICIAL_URL, '#website'),
      '@type': 'WebSite',
      'description': pkg.description,
      'inLanguage': DEFAULT_LANG,
      'name': brandName || BRANDING_NAME,
      'publisher': {
        '@id': this.getId(OFFICIAL_URL, '#organization'),
      },
      'url': OFFICIAL_URL,
  ...
  genArticle({
    ...
    const baseInfo: any = {
      ...
      'datePublished': dateCreated,
      'description': description,
      'headline': this.fixTitle(title, brandName),
  ...
  private fixTitle(title: string, brandName?: string) {
    const name = brandName || BRANDING_NAME;
