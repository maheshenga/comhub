...
import { resolvePlatformCredentials } from './platformProvider';
import { createVipCreditHooks } from './vipCreditHooks';
...
export const initModelRuntimeFromDB = async (
  ...
  // 4. Check if VIP user should be routed through platform NewAPI gateway.
  //    This only activates when: gateway is configured, user has no own API key, user is VIP.
  const userHasOwnApiKey = !!payload.apiKey;
  const platformCreds = await resolvePlatformCredentials(db, userId, userHasOwnApiKey);
  if (platformCreds) {
    // Route through NewAPI: override apiKey and baseURL, use openai-compatible SDK
    payload.apiKey = platformCreds.apiKey;
    payload.baseURL = platformCreds.baseURL;
  }

  // 5. Get business hooks (billing in cloud, undefined in OSS)
  const businessHooks = getBusinessModelRuntimeHooks(userId, provider);

  // 6. If user is routed through platform gateway, attach VIP credit deduction hooks
  const vipHooks = platformCreds ? createVipCreditHooks(db, userId) : undefined;

  // Merge hooks: VIP hooks wrap around business hooks
  const hooks: ModelRuntimeHooks | undefined = vipHooks
    ? {
        ...businessHooks,
        beforeChat: async (payload, options) => {
          await businessHooks?.beforeChat?.(payload, options);
          await vipHooks.beforeChat!(payload, options);
        },
        onChatFinal: async (data, context) => {
          await businessHooks?.onChatFinal?.(data, context);
          await vipHooks.onChatFinal!(data, context);
        },
        onChatError: async (error, context) => {
          await businessHooks?.onChatError?.(error, context);
          await vipHooks.onChatError!(error, context);
        },
      }
    : businessHooks;
