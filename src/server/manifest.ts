...
export class Manifest {
  public generate({
    ...
    if (brandLogoUrl) this._brandLogoUrl = brandLogoUrl;
    return {
  ...
  private _brandLogoUrl?: string;

  private _getImage = (url: string, version: number = 1) => ({
    cache_busting_mode: 'query',
    immutable: 'true',
    max_age: MAX_AGE,
    src: qs.stringifyUrl({ query: { v: version }, url: this._brandLogoUrl || BRANDING_LOGO_URL || url }),
  });
