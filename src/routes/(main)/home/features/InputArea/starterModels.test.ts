...
describe('starter models', () => {
  it('uses the admin-managed NewAPI provider in business builds instead of LobeHub', () => {
    ...
    expect(NEW_CHAT_PROVIDER).toBe(ENABLE_BUSINESS_FEATURES ? 'newapi' : 'anthropic');
    expect(NEW_MINIMAX_PROVIDER).toBe(ENABLE_BUSINESS_FEATURES ? 'newapi' : 'minimax');
  });
