...
type AdminBulkActionFlowProps = Omit<ButtonProps, 'children' | 'onClick'> & {
  actionId: AdminDangerousActionId;
  children: ReactNode;
  confirmDescription?: ReactNode;
  confirmTitle: ReactNode;
  count: number;
  onRun: (input: Required<Pick<AdminDangerousActionConfirmationInput, 'confirmed'>> &
    AdminDangerousActionConfirmationInput) => Promise<unknown>;
...
const AdminBulkActionFlow = memo(
  ({
    actionId,
    children,
    confirmDescription,
    ...
    const run = async () => {
      ...
      try {
        ...
        setErrorMessage(error instanceof Error ? error.message : t('admin.bulkAction.errorFallback', '操作失败'));
        setStep('error');
      }
    };
    ...
    const footer =
      step === 'confirm'
        ? [
            ...
            <Button key="run" danger={buttonProps.danger} type="primary" onClick={run}>
    ...
    return (
      <>
        <Button {...buttonProps} disabled={buttonProps.disabled || count <= 0} onClick={() => setOpen(true)}>
          {children}
        </Button>
        <Modal
        ...
        >
          {step === 'confirm' ? (
            <Flexbox gap={12}>
              ...
              {requirement?.requiresReason || reasonOptional ? (
                <Input.TextArea
            ...
            <Flexbox align="center" gap={16}>
              ...
              <Typography.Text>
                {progressDescription ?? t('admin.bulkAction.progress', '正在执行批量操作，请勿关闭页面。')}
              </Typography.Text>
            </Flexbox>
          ) : step === 'done' ? (
            <Flexbox gap={12}>
              <Alert message={resultSummary?.title ?? t('admin.bulkAction.done', '批量操作已完成')} type="success" />
              {renderSummaryCounts()}
...
export default AdminBulkActionFlow;
