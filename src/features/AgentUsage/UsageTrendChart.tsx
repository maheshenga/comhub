...
const UsageTrendChart = memo<UsageTrendChartProps>(({ buckets, isLoading }) => {
  ...
  const chartData = useMemo(
    () =>
      (buckets ?? []).map((bucket) => ({
        [cacheWriteKey]: type === ShowType.Spend ? bucket.cacheWriteCost : bucket.cacheWriteTokens,
        [inputKey]: type === ShowType.Spend ? bucket.inputCost : bucket.inputTokens,
        [outputKey]: type === ShowType.Spend ? bucket.outputCost : bucket.outputTokens,
        label: bucket.label,
      })),
